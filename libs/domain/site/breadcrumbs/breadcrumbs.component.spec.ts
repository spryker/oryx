...
import { Breadcrumb, BreadcrumbsService, siteProviders } from '@spryker-oryx/site';
...
describe('SiteBreadcrumbsComponent', () => {
  ...
  beforeEach(async () => {
    ...
    service = injector.inject<MockBreadcrumbsService>(
      BreadcrumbsService
    );
  });
  ...
  describe('when breadcrumbs are not provided', () => {
    beforeEach(async () => {
      element = await fixture(html`<oryx-site-breadcrumbs></oryx-site-breadcrumbs>`);
    });
  ...
  describe('when breadcrumbs are provided', () => {
    ...
    describe('and breadcrumb has a text i18n token', () => {
      ...
      it('should translate the token', () => {
        expect(spy).toHaveBeenCalledWith(breadcrumbI18n.i18n?.token, breadcrumbI18n.i18n?.values);
      });
    });
    ...
    describe('and dividerIcon option is provided', () => {
      ...
      it('should icon type to the icon', () => {
        const divider = element.renderRoot.querySelector<IconComponent>('oryx-icon');
