...
describe('FiltersComponent', () => {
  ...
  beforeEach(async () => {
    ...
    element = await fixture(html`<oryx-picking-filters></oryx-picking-filters>`);
  });
  ...
  it('should build the form', () => {
    expect(renderer.buildForm).toHaveBeenCalledWith(fields, {sortBy: 'deliveryDate.desc'})
  })

  describe('when qualifier changes', () => {
    beforeEach(async () => {
      service.getSortingQualifier = vi.fn().mockReturnValue(of({
        ...defaultSortingQualifier,
        sortDesc: false,
      }))
      element = await fixture(html`<oryx-picking-filters open></oryx-picking-filters>`);
    })

    it('should format the correct value', () => {
      expect(renderer.buildForm).toHaveBeenCalledWith(fields, {sortBy: 'deliveryDate.asc'})
    })
  });
  ...
  describe('when filters are opened', () => {
    ...
    describe('and modal is closed', () => {
      ...
      beforeEach(() => {
        ...
        element.renderRoot.querySelector('oryx-modal')?.dispatchEvent(
    ...
    describe('and filters are applied', () => {
      ...
      beforeEach(async () => {
        ...
        element.renderRoot.querySelector('oryx-button[slot="footer"] button')?.dispatchEvent(
