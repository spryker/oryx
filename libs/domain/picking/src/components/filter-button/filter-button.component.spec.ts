...
describe('FilterButtonComponent', () => {
  ...
  beforeEach(async () => {
    ...
    element = await fixture(html`<oryx-picking-filter-button></oryx-picking-filter-button>`);
  });
  ...
  describe('when selected sort by method is not default', () => {
    beforeEach(async () => {
      ...
      service.getSortingQualifier = vi.fn().mockReturnValue(of({
        ...defaultSortingQualifier, sortBy: 'test'
      }))
      element = await fixture(html`<oryx-picking-filter-button></oryx-picking-filter-button>`);
  ...
  describe('when selected sorting order is not default', () => {
    beforeEach(async () => {
      service.getSortingQualifier = vi.fn().mockReturnValue(of({
        ...defaultSortingQualifier, sortDesc: false
      }))
      element = await fixture(html`<oryx-picking-filter-button></oryx-picking-filter-button>`);
    })
