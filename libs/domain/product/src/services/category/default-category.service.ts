...
import { BehaviorSubject, Observable, Subject, combineLatest, flatMap, forkJoin, map, merge, mergeMap, of, switchMap } from 'rxjs';
...
export class DefaultProductCategoryService implements ProductCategoryService {
  ...
  getTrail(categoryId: string): Observable<ProductCategory[]> {
    return this.get(categoryId).pipe(
      switchMap(category =>
        category.parent ? 
        this.getTrail(category.parent).pipe(map(trail => [...trail, category]))
        : of([category])
      ),
  ...
  protected fetchCategory(categoryId: string): void {
    ...
    this.adapter.get(categoryId).subscribe(category => this.categories.get(categoryId)!.next(category));
  }
}
