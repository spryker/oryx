...
const Template: Story<Props> = (): TemplateResult => {
  const renderButton = (
    ...
    return html` <h1>Critical outline with icon</h1>
      <div class="button-component">
        <p>Default</p>
        ${Object.values(set).map(
          (size) =>
            html`
              <oryx-button type="critical" size=${size} outline icon>
                <button>
        ...
        ${Object.values(set).map(
          (size) =>
            html`
              <oryx-button size=${size} icon outline>
                <button disabled>
                  <oryx-icon type=${icon}></oryx-icon>
                  ${message}
                </button>
        ...
        ${Object.values(set).map(
          (size) =>
            html`
              <oryx-button loading type="critical" size=${size} outline icon>
...
export const ButtonCriticalOutlineWithIcon = Template.bind({});
