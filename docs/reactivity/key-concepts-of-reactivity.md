...
1. `ProductTitleComponent` is a web component that renders the title in the DOM. The title is typically an `<h1>` element, but this is configurable to make the component reusable in another context, for example, inside a cart entry component. The product title component does nothing about the context (e.g. PDP vs cart entry), but relies on a controller to get the context and associated product data. The product title (`name`) is mapped from the product data.
