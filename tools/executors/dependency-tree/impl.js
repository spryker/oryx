...
async function depsExecutor(options, context) {
    ...
    if (update) {
        ...
        (0, fs_1.writeFileSync)(`apps/${projectName}/dependencies.sh`, `DEPENDENCIES="${dependencies}"`);
        return { success: true };
    }
    console.log(dependencies);
    console.info(`Checking if dependencies match...`);
    const realDependencies = (0, fs_1.readFileSync)(`apps/${projectName}/dependencies.sh`, {
        encoding: 'utf8',
        flag: 'r',
    }).match(/"([^"]+)"/)[1];
    console.log(realDependencies);
    if (dependencies !== realDependencies) {
        console.log(`Defined dependencies do not match with the current ones, please generate them with "npx nx deps ${projectName} --update"`);
